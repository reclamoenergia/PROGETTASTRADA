...
class DxfExporter:
    ...
    def export_all_layout(
        ...
        if profile and profile.progressive:
            profile_sheet_count = self._draw_profile_sheets(msp, sheet, profile, sections)
    ...
    def _draw_profile_sheets(self, msp, sheet: SheetSpec, profile: ProfileData, sections: List[SectionData]) -> int:
        p0 = profile.progressive[0]
        p1 = profile.progressive[-1]
        span_per_sheet = (sheet.usable_width - 40.0) * self.PROFILE_H_SCALE / 1000.0
        ...
        for idx in range(sheet_count):
            ...
            scale_txt = f"Scale H 1:{int(self.PROFILE_H_SCALE)}  V 1:{int(self.PROFILE_V_SCALE)}"
            ...
            marks = [s for s in sections if s_prog <= s.progressive <= e_prog]
            for sec in marks:
            ...
            self._draw_profile_table(msp, marks, profile, graph_left, graph_right, table_top, usable_bottom + 20.0)
        return sheet_count

    def _draw_profile_table(self, msp, marks: List[SectionData], profile: ProfileData, left: float, right: float, top: float, bottom: float) -> None:
        rows = ["PROGRESSIVA", "QUOTA TERRENO", "QUOTA PROGETTO"]
        height = top - bottom
        row_h = height / max(1, len(rows))
        label_w = 45.0
        n = max(1, len(marks))
        col_w = max(20.0, (right - left - label_w) / n)
        table_right = left + label_w + col_w * n
        msp.add_lwpolyline(
            [(left, top), (table_right, top), (table_right, bottom), (left, bottom), (left, top)],
            dxfattribs={"layer": "PROF_TABLE", "closed": True},
        )
        msp.add_line((left + label_w, top), (left + label_w, bottom), dxfattribs={"layer": "PROF_TABLE"})
        for ridx in range(1, len(rows)):
            y = top - ridx * row_h
            msp.add_line((left, y), (table_right, y), dxfattribs={"layer": "PROF_TABLE"})
        for c in range(1, n):
        ...
        for ridx, label in enumerate(rows):
            ...
            msp.add_text(label, dxfattribs={"height": 2.2, "layer": "PROF_TEXT"}).set_placement((left + 1.5, y))

        for idx, sec in enumerate(marks):
            terr = self._interp_piecewise(profile.progressive, profile.terrain_z, sec.progressive)
            proj = self._interp_piecewise(profile.progressive, profile.project_z, sec.progressive)
            x = left + label_w + idx * col_w + 1.0
            values = [f"{sec.progressive:.2f}", f"{terr:.2f}", f"{proj:.2f}"]
            for ridx, val in enumerate(values):
                y = top - (ridx + 0.7) * row_h
                msp.add_text(val, dxfattribs={"height": 2.2, "layer": "PROF_TEXT"}).set_placement((x, y))
    ...
    def _draw_section_sheets(
        ...
        if not sections:
            return
        prepared = [self._prepare_section_layout(s, quote_step_m, z_exaggeration) for s in sections]
    ...
    def _prepare_section_layout(self, section: SectionData, quote_step_m: float, z_exaggeration: float) -> Optional[dict]:
        ...
        graph_w = max(120.0, (x_max - x_min) * 1000.0 / self.SECTION_H_SCALE + 20.0)
        graph_h = max(70.0, (z_max - z_min) * 1000.0 / self.SECTION_H_SCALE * z_exaggeration + 20.0)
        head_h = 22.0
        table_h = 44.0
        cart_w = graph_w + 16.0
    ...
    def _draw_single_section_cartiglio(self, msp, item: dict, x0: float, y0: float, min_width: float) -> None:
        ...
        hdr = (
            ...
            f"Scala H 1:{int(self.SECTION_H_SCALE)} Vx{item['z_exaggeration']:.2f}"
        )
        ...
        self._draw_section_table(msp, item["points"], x0 + 8.0, x1 - 8.0, y0 + item["table_h"] + 2.0, y0 + 3.0)

    def _draw_section_table(self, msp, points: List[dict], left: float, right: float, top: float, bottom: float) -> None:
        rows = ["OFFSET", "TERRENO", "PROGETTO"]
        n = max(1, len(points))
        label_w = 22.0
        row_h = (top - bottom) / len(rows)
        col_w = max(13.0, (right - left - label_w) / n)
        table_right = left + label_w + col_w * n
        ...
        for ridx, row in enumerate(rows):
            y = top - (ridx + 0.7) * row_h
            msp.add_text(row, dxfattribs={"height": 1.8, "layer": "SEZ_TEXT"}).set_placement((left + 0.8, y))

        for cidx, p in enumerate(points):
            x = left + label_w + cidx * col_w + 0.8
            vals = [
                f"{p['offset']:.2f}",
            ...
            for ridx, val in enumerate(vals):
                y = top - (ridx + 0.7) * row_h
                msp.add_text(val, dxfattribs={"height": 1.8, "layer": "SEZ_TEXT"}).set_placement((x, y))
