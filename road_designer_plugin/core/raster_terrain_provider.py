...
class RasterTerrainProvider(TerrainProvider):
    ...
    def get_elevation(self, x: float, y: float) -> float:
        ...
        if not self.raster.extent().contains(pt):
            return float("nan")
        result = self.provider.identify(pt, QgsRaster.IdentifyFormatValue)
    ...
    def sample_many(self, points: Iterable[Tuple[float, float]]) -> List[float]:
        samples = [self.get_elevation(x, y) for x, y in points]
